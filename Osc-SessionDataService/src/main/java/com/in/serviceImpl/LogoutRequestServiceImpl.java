...
public class LogoutRequestServiceImpl implements LogoutRequestService {
    ...
    public LogoutResponseDto logoutUser(LogoutRequestDto logoutRequest) {
        ...
        // Fetch session data, throw exception if not found
        ...
        // Check if the session is already logged out or not
        if (sessionData.getLogoutTime() == null && sessionData.getLoginTime() != null) {
            sessionData.setLogoutTime(LocalDateTime.now()); // Set logout time
            sessionRepository.save(sessionData); // Persist updated session entity
